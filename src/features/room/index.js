...
function EditToolbar(props) {
  ...
  const handleClick = () => {
    setRows((oldRows) => [
      ...oldRows,
      { id, room_number: "", floor: "", seat: "", comment: "", status: "" },
    ]);
...
export default function University() {
  ...
  const handleDeleteClick = (id) => () => {
    axios.delete(`${Base_URL}/api/deleteRoom/${id}`).then((res) => {
  ...
  const processRowUpdate = (newRow) => {
    ...
    if (isNaN(newRow.id)) {
      axios.post(`${Base_URL}/api/createRoom`, newRow).then((res) => {
        getListRoom(setRows);
      });
    } else {
      axios.put(`${Base_URL}/api/editRoom/${newRow.id}`, newRow).then((res) => {
        getListRoom(setRows);
      });
    }
